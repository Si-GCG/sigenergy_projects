...
# --- Helper functions for register access (slave/unit compatibility) ---
async def read_input_registers(client, address, count, slave):
    return await client.read_input_registers(address=address, count=count, slave=slave)

async def read_holding_registers(client, address, count, slave):
    return await client.read_holding_registers(address=address, count=count, slave=slave)

async def write_register(client, address, value, slave):
    return await client.write_register(address=address, value=value, slave=slave)

async def write_registers(client, address, values, slave):
    return await client.write_registers(address=address, values=values, slave=slave)
            ...
            async with self._locks[key]:
                with _suppress_pymodbus_logging(really_suppress= False if _LOGGER.isEnabledFor(logging.DEBUG) else True):
                    result = await read_input_registers(
                        ...
                        else await read_holding_registers(
                        client,
                        address=address,
                        count=count,
                        slave=slave_id
